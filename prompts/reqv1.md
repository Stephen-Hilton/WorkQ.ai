# RequestQueue.ai — Requirements (v1)
...
- **SSM Parameter Store** — `/requestqueue/email_whitelist`, the canonical email whitelist for sign-up.
- *(Secrets Manager is intentionally NOT used. The original design stored the service-user password there; we eliminated it — see the addendum to DECISION 9 below — and now require the user to supply the password via `REQUESTQUEUE_SERVICE_USER_PASSWORD` in `.env` directly.)*
...
*(DECISION 9: runtime has zero AWS IAM credentials — only a Cognito service-user password supplied via `REQUESTQUEUE_SERVICE_USER_PASSWORD` in `.env`. Generate / rotate via `scripts/refresh_creds.sh`.)*
...
- **Deploy inputs (`.env`, hand-edited, gitignored):**
  - `REQUESTQUEUE_AWS_REGION` — must be `us-east-1` if using a custom domain (CloudFront cert constraint).
  - `REQUESTQUEUE_AWS_ACCESS_KEY_ID` / `REQUESTQUEUE_AWS_SECRET_ACCESS_KEY` — used only for `sam deploy`. Not needed at local-server runtime.
  - `REQUESTQUEUE_SERVICE_USER_PASSWORD` — Cognito service-user password (16+ chars). Generated via `scripts/refresh_creds.sh`; passed to SAM as a `NoEcho` CloudFormation parameter on each deploy.
  - `REQUESTQUEUE_CUSTOM_DOMAIN` (optional) — e.g., `example.com`. If set, webapp deploys to `work.<domain>` and API to `api.<domain>`.
  - `REQUESTQUEUE_CUSTOM_DOMAIN_CERT_ARN` (optional, required if `REQUESTQUEUE_CUSTOM_DOMAIN` is set) — single ACM cert in `us-east-1` covering both subdomains (wildcard or SAN).
  - `REQUESTQUEUE_EMAIL_WHITELIST` — comma-separated emails or `@domain` entries; seeds the SSM whitelist on first deploy.
  - `REQUESTQUEUE_GITHUB_REPO_URL` — the repo claude will operate on.
  - `REQUESTQUEUE_GITHUB_BRANCH` — default `main`.
  - `REQUESTQUEUE_GITHUB_TOKEN` — fine-grained PAT with `contents:write` and `pull_requests:write` (plus admin if auto-merge enabled).
  - `REQUESTQUEUE_GITHUB_AUTO_MERGE` — `false` by default. If `true`, every PR is auto-merged via `gh pr merge --admin` immediately after creation. *(DECISION 12.)*
  - `REQUESTQUEUE_GITHUB_AUTO_MERGE_METHOD` — `squash` (default) | `merge` | `rebase`.
  - `REQUESTQUEUE_POLLING_SECONDS` — monitor poll interval; default 30.
  - `REQUESTQUEUE_BUILD_TIMEOUT_SECONDS` — max claude wall-clock; default 2700 (45 min). *(DECISION 9b.)*
  - `REQUESTQUEUE_DISPLAY_TIMEZONE` — display-only TZ for timelog. UTC is always used for storage. Default `UTC`. Use abbreviations from <https://en.wikipedia.org/wiki/List_of_time_zone_abbreviations>.
  - `REQUESTQUEUE_PROMPT_PARTS_PATH` — path on disk; default `./config/prompt_parts.yaml`.

- **Deploy outputs (`.requestqueue.outputs.json`, auto-written by `sam deploy`, gitignored):**
...
all:
  ...
  post: |
    ...
    complete, end your response with the exact line `<!-- requestqueue:status=pending_review -->`.
    If you tried but could not satisfy the request, end with `<!-- requestqueue:status=failed -->`.
...
- **`reqcreator`** — derived from the JWT `email` claim on POST. **Immutable** after creation; PUT silently strips this field. Webapp users → their own email; service user → `service-local-monitor@requestqueue.internal`. *(DECISION 19.)*
...
- **`timelog`** — list of objects: `[{"status": "<status>", "ts": "<iso8601-utc>"}, ...]`. Append-only. Webapp converts UTC → `REQUESTQUEUE_DISPLAY_TIMEZONE` for display. The list length doubles as the version vector for optimistic concurrency on PUT. *(DECISIONS 16, 21.)*
...
2. User signs up with their email. Pre-signup Lambda checks SSM whitelist (`/requestqueue/email_whitelist`); rejects if no match. Wildcard `@domain.com` entries match anyone in that domain. *(DECISION 8.)*
...
- **Body:**
  ...
  - Each header shows: `reqstatus`, `reqarea`, first line of `request` (as a title proxy), first `timelog` entry's timestamp (in `REQUESTQUEUE_DISPLAY_TIMEZONE`), and an Actions context menu.
  - **Actions menu:**
  ...
  - Expanding a row shows all DDB fields:
    ...
    - `timelog` at the bottom (read-only), all timestamps in `REQUESTQUEUE_DISPLAY_TIMEZONE`.
...
1. `local/monitor` wakes up every `REQUESTQUEUE_POLLING_SECONDS`.
2. Calls `GET /status/queued`; sorts results by `reqid` (uuid v7 = create-order).
3. Also calls `GET /status/all` and scans for stuck records: any in `building` or `planning` whose latest `timelog.ts` is older than `REQUESTQUEUE_BUILD_TIMEOUT_SECONDS + 60s`. Force-sets those to `failed` with a "Build appears to have died" response, including a `# Recommended Next Step` section. *(DECISION 15.)*
4. For each queued record, top to bottom:
   ...
   2. `local/build`:
      ...
      - Wall-clock timeout: `REQUESTQUEUE_BUILD_TIMEOUT_SECONDS` (default 45 min). On timeout: kill, status=`failed`, response includes last 100 lines of output.
      - For `build` requests:
        - If claude made commits: `git push origin requestqueue/<reqid>`, `gh pr create`, capture PR URL into `reqpr`. If `REQUESTQUEUE_GITHUB_AUTO_MERGE=true`: also `gh pr merge --<method> --delete-branch --admin`.
      ...
      - Parses last 500 bytes of stdout for `<!-- requestqueue:status=<value> -->` fence. Whitelist: `pending_review`, `complete`, `failed`. *(DECISION 14.)*
...
5. After draining the queue, monitor sleeps for `REQUESTQUEUE_POLLING_SECONDS`.
...
- **Local server:** authenticates as Cognito service user `service-local-monitor`. Password lives in `REQUESTQUEUE_SERVICE_USER_PASSWORD` in the local server's `.env`. Generated by `scripts/refresh_creds.sh` on the deploy machine and either reused (single-machine) or copy-pasted to the local server (two-machine). Runtime has zero AWS IAM credentials. *(DECISION 9.)*
...
- **First deploy (developer's machine):**
  ...
  2. `scripts/refresh_creds.sh` — generates `REQUESTQUEUE_SERVICE_USER_PASSWORD` and writes it (with backup) into `.env`.
  3. `make publish` (runs `sam deploy`, `pnpm build`, `aws s3 sync`, runtime config upload, CloudFront invalidation).
  4. `scripts/whitelist_user.sh -a yourname@yourdomain.com`.
- **Local server setup (one-time, on the box that runs claude — skip if same as deploy machine):**
  1. `git clone` + `cp .env.example .env`, fill in.
  2. Copy the two `REQUESTQUEUE_SERVICE_USER_*` lines from the deploy machine's `.env`.
  3. `python -m monitor` (or `make monitor`) — long-running.
...
7. **CloudFront + optional unified custom domain.** `REQUESTQUEUE_CUSTOM_DOMAIN=example.com` → `work.<d>` + `api.<d>` with one ACM cert. Inputs in `.env`, outputs in `.requestqueue.outputs.json`.
8. **Cognito whitelist via SSM Parameter Store.** Wildcards `@domain.com`. Helper `scripts/whitelist_user.sh -a/-r/-l/-h`.
9. **Cognito for both webapp and local.** Service user `service-local-monitor` with a user-supplied password (`REQUESTQUEUE_SERVICE_USER_PASSWORD` in `.env`). The original design stored the password in AWS Secrets Manager; this was later eliminated because the password ends up in `.env` regardless and Secrets Manager added ~$0.40/mo + a fetch step + an extra service in the IAM permission set for no security benefit. The password now flows: `.env` → `make publish` → SAM `NoEcho` parameter → custom resource → Cognito user. `scripts/refresh_creds.sh` generates / rotates the password (with `.env` backup). Token TTL 24h/30d. Build timeout 45 min.
...
12. **`git worktree`-per-build.** Branch `requestqueue/<reqid>`; always create PR; optional auto-merge with `--admin`.
13. **`reqcommit` renamed to `reqpr`.** Stores PR URL.
14. **Status fence** `<!-- requestqueue:status=<value> -->` (whitelist: `pending_review`, `complete`, `failed`). New `failed` status added.
15. **Failure handling.** Uniform mapping; every failure response has a `# Recommended Next Step`; stuck-build detector in monitor.
16. **`timelog` is list of objects.** UTC stored; displayed in `REQUESTQUEUE_DISPLAY_TIMEZONE`.
