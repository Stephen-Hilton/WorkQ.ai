...
- **Secrets Manager** — `/requestqueue/service-local-monitor/password`, the password for the Cognito service user used by the local server.
...
- **Deploy inputs (`.env`, hand-edited, gitignored):**
  ...
  - `REQUESTQUEUE_AWS_ACCESS_KEY_ID` / `REQUESTQUEUE_AWS_SECRET_ACCESS_KEY` — used only for `sam deploy` and `scripts/bootstrap_local.sh`. Not needed at local-server runtime.
...
- **Local server:** authenticates as Cognito service user `service-local-monitor`. Password fetched once via `scripts/bootstrap_local.sh` to `~/.config/requestqueue/credentials`. Runtime has zero AWS IAM credentials. *(DECISION 9.)*
