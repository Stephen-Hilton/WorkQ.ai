...
High-level flow:
  ...
  3. Set up a git worktree on `requestqueue/<reqid>` (build only; planning skips git).
    ...
    try:
        if is_build:
            ...
            try:
                git_ops.ensure_bare_clone(repo_url=config.github_repo_url, bare_dir=bare_dir)
                worktree_dir = config.repo_root / "local" / "workspace" / reqid
                branch = f"requestqueue-plan/{reqid}"
                git_ops.add_worktree(
                    bare_dir=bare_dir,
                    worktree_dir=worktree_dir,
                    base_branch=config.github_branch,
...
def _next_steps_timeout(reqid: str, branch: str | None, timeout: int) -> str:
    return (
        ...
        f"`REQUESTQUEUE_BUILD_TIMEOUT_SECONDS` in `.env` (max ~24h, but most of "
...
def _next_steps_auto_merge_failed(pr_url: str, err: str) -> str:
    return (
        ...
        f"- Verify your `REQUESTQUEUE_GITHUB_TOKEN` has admin rights on the repo.\n"
...
def _pr_title(record: dict) -> str:
    ...
    first = req[0].strip() if req else f"RequestQueue build {record.get('reqid', '')}"
    if len(first) > 70:
        first = first[:67] + "…"
    return f"requestqueue: {first}"


def _pr_body(record: dict) -> str:
    return (
        f"Generated by RequestQueue.ai for request `{record.get('reqid', '')}`.\n\n"
        f"**Original request:**\n\n"
        f"```\n{(record.get('request') or '').strip()}\n```\n\n"
        f"_See the RequestQueue webapp for the full conversation and AI response._"
    )
